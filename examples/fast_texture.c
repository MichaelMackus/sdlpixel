...
int main()
{
    ...
    SDL_Texture* texture = SDL_CreateTexture(s, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, windowWidth, windowHeight);
    ...
    // TODO see for ideas: https://lazyfoo.net/tutorials/SDL/42_texture_streaming/index.php
    while (!quit) {
        while (SDL_PollEvent(&event)) {
            ...
            for (int y=0; y<windowHeight; ++y) {
                for (int x=0; x<windowWidth; ++x) {
                    pixels[x + y*pitch/4] = r << 24 | g << 16 | b << 8 | 0xFF;
                    ++r;
                    --g;
                    b ^= b;
                    ++b;
            ...
            SDL_SetRenderDrawColor(s, 0x00, 0x00, 0x00, 0x00);
            SDL_RenderClear(s);
            SDL_RenderCopyEx(s, texture, NULL, &dest, 0, NULL, SDL_FLIP_NONE);
            SDL_SetRenderDrawColor(s, 0xFF, 0xFF, 0xFF, 0xFF);
            SDL_RenderDrawLine(s, 10, 10, 500, 500);
            SDL_RenderPresent(s);
